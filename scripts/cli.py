...
def cmd_locate_comment(args: argparse.Namespace) -> None:
    """打开帖子并定位、高亮评论，不发布内容。"""
    from xhs.comment import locate_comment
    ...
    try:
        ...
        _output({"success": True, "message": "评论已定位并高亮"})
    finally:
        browser.close()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # locate-comment
    sub = subparsers.add_parser("locate-comment", help="定位并高亮评论，不发布内容")
    sub.add_argument("--feed-id", required=True)
    sub.add_argument("--xsec-token", required=True)
    sub.add_argument("--comment-id")
    sub.add_argument("--user-id")
    sub.set_defaults(func=cmd_locate_comment)
