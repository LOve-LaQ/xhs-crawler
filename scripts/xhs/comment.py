...
def locate_comment(
    ...
    if not _find_and_scroll_to_comment(page, comment_id, user_id):
        raise RuntimeError(f"未找到评论 (commentID: {comment_id}, userID: {user_id})")

    selector = f"#comment-{comment_id}" if comment_id else ""
    highlighted = page.evaluate(
        f"""
        (() => {{
            let target = {json.dumps(selector)}
                ? document.querySelector({json.dumps(selector)})
                : null;
            if (!target && {json.dumps(user_id)}) {{
                const userId = {json.dumps(user_id)};
                target = [...document.querySelectorAll('.parent-comment, .comment-item, .comment')]
                    .find(el => el.querySelector(`[data-user-id="${{userId}}"]`));
            }}
            if (!target) return false;
            const styleId = 'xhs-insight-comment-highlight';
            let style = document.getElementById(styleId);
            if (!style) {{
                style = document.createElement('style');
                style.id = styleId;
                style.textContent = `.${{styleId}} {{
                    outline: 3px solid #f45d52 !important;
                    outline-offset: 4px;
            ...
            document.querySelectorAll('.' + styleId).forEach(el => el.classList.remove(styleId));
    ...
    if not highlighted:
        raise RuntimeError("已找到评论，但页面未能注入高亮")
